...
namespace Searching
{
    public class Leaderboard : MonoBehaviour
    {
        ...
        {
            // sequential search if the player is already in the list
            int index = -1;
            ...
            // find index to insert that make the scores list sorted with binary search
            index = -1; // reset index
            int left = 0;
            ...
            // If the score is not found, insert it at the appropriate index
            if (index == -1)
            {
                index = left;
            }
